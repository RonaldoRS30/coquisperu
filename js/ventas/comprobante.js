                ...
                switch (data.result){
                    case 'success':
                        ...
                        if (tipo_oper == "V"){
                            ...
                            img_estadoHtml = '<a onclick="abrirAnulacionModal('+comprobante+',\''+data.serie+'\',\''+data.numero+'\',\''+data.total+'\');" href="javascript:;" class="enviarcorreo"><img src="' + base_url + 'images/active.png" width="16" height="16" border="0" title="Anular documento"></a>';
                            ...
                            if ( tipo_oper == "V" && tipo_docu != "N"){
                                ...
                                xmlHTML  = '<a href="javascript:;" onclick="abrir_xml_envioSunat(' + comprobante + ')" target="_parent"><img src="' + base_url + 'images/xml.png" width="16" height="16" border="0" title="XML sunat"></a>';
                                $(".pdfSunat_data_"+pos).html(compHTML);
                                $(".xmlSunat_data_"+pos).html(xmlHTML);
                            }else{
                                $(".disparador_data_"+pos).html("Aprobado");
                            }
...
function canjeToGuia(comprobante, pos){
    ...
    Swal.fire({
        title: 'Desea generar la guía?',
        text: "Verifique si no se ha generado anteriormente!",
        icon: 'warning',
        showCancelButton: true,
                ...
                success: function (data) {
                    switch (data.result){
                        case 'success':
                            Swal.fire(
                                'Guía Generada!',
                                'La guia se encuentra asociada a esta factura.',
                                'success'
                            )
...
function recalcular_montoFP(){
    ...
    importe_total = parseFloat($("#importetotal").val());
    ...
    total_montoP = importe_total-total_FP;
    
    $('#montoFP_default').val(total_montoP.toFixed(2))
   
}

function limpiar_montoFP(){
    var otherMontos         = $('#tbFormasPago').find('tbody tr').find('.monto');
    var cmbFormasPago       = $('#tbFormasPago').find('tbody tr').find('.cmbFormasPago');
    ...
    $(cmbFormasPago).each(function (i, item)
    {
        if ($(item).val() > 0)
            $(item).val("");
    });
