...
class Ocompra extends Controller{
	...
	## Dev: LV -> Begin - El array somevar es reemplazado por atributos
	private $empresa;
	private $compania;
	private $usuario;
	private $rol;
    ...
    if ($error == false){
	    ...
	    if ($detalle != NULL) {
	      foreach ($detalle as $indice => $valor) {
	        ...
	        $filterGuia->flagBS                = $valor->flagBS;
          $filterGuia->PRODCTOP_Codigo       = $valor->PROD_Codigo;
	        ...
	        $filterGuia->GUIAREMDETC_Peso      = $valor->PROD_Peso;
  ...
  public function insertar_comprobante(){
    ...
    if ($error == false){
      ...
      ## PAGO Y TOTAL
        ...
        $filter->FORPAP_Monto     = $datos_ocompra[0]->OCOMC_total;
      ...
      ## ARTICULOS
        ...
        if (count($detalle) > 0) {
          foreach ($detalle as $indice => $valor) {        
            ...
            $filterDet->PROD_Peso           = $valor->PROD_Peso;
  ...
  ## Dev: LV -> End
    ...
    public function nueva_ocompra($tipo_oper = 'C',$sucursal=''){
        ...
        $oculto = form_hidden(array('accion' => $accion, 'codigo' => $codigo, 'empresa' => '', 'persona' => '', 'modo' => $modo, 'base_url' => base_url(), 'tipo_oper' => $tipo_oper));
    ...
    public function insertar_ocompra(){
        ...
        $filter->CENCOSP_Codigo   = $this->input->post('contiene_igv');
    ...
    {   
        ...
        $proveedor        = $datos_ocompra[0]->PROVP_Codigo;
        $cliente          = $datos_ocompra[0]->CLIP_Codigo;
        $contiene_igv     = $datos_ocompra[0]->CENCOSP_Codigo;
        $moneda           = $datos_ocompra[0]->MONED_Codigo;
        ...
        $oculto                   = form_hidden(array('accion' => $accion, 'codigo' => $codigo, 'empresa' => $empresa, 'persona' => $persona, 'modo' => $modo, 'base_url' => base_url(), 'tipo_oper' => $tipo_oper));
        ...
        //DATOS PARA CREAR NUEVOS CLIENTES
          $data["categorias_cliente"] = $this->tipocliente_model->listar();
          $data["documentosNatural"]  = $this->tipodocumento_model->listar_tipo_documento();
          $data["documentosJuridico"] = $this->tipocodigo_model->listar_tipo_codigo();
        //FIN DATOS PARA CREAR NUEVOS CLIENTES
