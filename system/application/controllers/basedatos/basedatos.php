...
class basedatos extends Controller{
	...
	public  function add_clientecompania(){
		$this->eliminar_model->truncate_clientecompania();
		$empresa 		= $this->session->userdata('empresa'); 
		#TODAS LAS EMPRESAS
        $array_compania = $this->compania_model->listar(null);
        #SOLO EMPRESA ACTUAL
        //$array_compania = $this->compania_model->listar_establecimiento($empresa);   
		foreach ($array_compania as $key => $value){
			$sql = ' INSERT INTO cji_clientecompania (CLIP_Codigo, COMPP_Codigo) SELECT c.CLIP_Codigo,"'.$value->COMPP_Codigo.'" FROM cji_cliente c';
	...
	public function add_productocompania($value='')
	{
		$empresa = $this->session->userdata('empresa'); 
		$this->eliminar_model->truncate_productocompania();
        #TODAS LAS EMPRESAS
        $array_compania = $this->compania_model->listar(null);
        #SOLO EMPRESA ACTUAL
        //$array_compania = $this->compania_model->listar_establecimiento($empresa);           
    	foreach ($array_compania as $key => $value){
        	$sql = ' INSERT INTO cji_productocompania (PROD_Codigo, COMPP_Codigo) SELECT p.PROD_Codigo,"'.$value->COMPP_Codigo.'" FROM cji_producto p';
        	$query = $this->db->query($sql);
    	}
	}
