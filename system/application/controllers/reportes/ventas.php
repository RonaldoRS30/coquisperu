...
class Ventas extends Controller {
    ...
    public function excel_producto_por_vendedor($vendedor = "", $fechai = NULL, $fechaf = NULL)
    {
        ...
        foreach ($vendedores as $key => $value) {
            ...
            $this->excel->setActiveSheetIndex($hoja)->mergeCells('A3:H3')->setCellValue("A3", "REPORTE DE VENTAS POR PRODUCTO SEGUN VENDEDOR. DESDE $f_ini HASTA $f_fin");
            ...
            $this->excel->getActiveSheet()->getStyle('A5:J5')->applyFromArray($estiloColumnasTitulo);
            ...
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("F$lugar", "VOLUMEN");
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("G$lugar", "VOLUMEN TOTAL");
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("H$lugar", "COSTO");
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("I$lugar", "COSTO T.");
            $this->excel->setActiveSheetIndex($hoja)->setCellValue("J$lugar", "TOTAL S/");
            ...
            if ($productosInfo > 0) {
                foreach ($productosInfo as $key => $value) {
                    ...
                    $this->excel->setActiveSheetIndex($hoja)->setCellValue("D$fila", $value['MARCC_CodigoUsuario']);
                    $this->excel->setActiveSheetIndex($hoja)->setCellValue("E$fila", $value['cantidadTotal']);

                    // Volumen sin number_format para que sea numérico
                    $this->excel->setActiveSheetIndex($hoja)->setCellValue("F$fila", $value['PROD_Volumen']);
                    // Aplica formato 4 decimales a la celda F
                    $this->excel->getActiveSheet()->getStyle("F$fila")->getNumberFormat()->setFormatCode('0.0000');
                    // Volumen total sin number_format
                    $this->excel->setActiveSheetIndex($hoja)->setCellValue("G$fila", $value['volumenTotal']);
                    // Aplica formato 4 decimales a la celda G
                    $this->excel->getActiveSheet()->getStyle("G$fila")->getNumberFormat()->setFormatCode('0.0000');
                    
                    $this->excel->setActiveSheetIndex($hoja)->setCellValue("H$fila", $value['PROD_UltimoCosto']);
                    $this->excel->setActiveSheetIndex($hoja)->setCellValue("I$fila", $value['PROD_UltimoCosto'] * $value['cantidadTotal']);
                    $this->excel->setActiveSheetIndex($hoja)->setCellValue("J$fila", $value['ventaTotal']);
                    $fila++;
                }
    ...
    public function productos_vendidos_detalle()
    {
        ...
        if ($reporte_result["records"] != NULL) {
            foreach ($reporte_result["records"] as $valor) {
                ...
                $valoru         = number_format($valor->CPDEC_Pu,2);
                $preciou        = number_format($valor->CPDEC_Pu_ConIgv,2);
                $subtotal       = number_format($valor->CPDEC_Subtotal,2);
                $igv            = number_format($valor->CPDEC_Igv,2);
                $total          = number_format($valor->CPDEC_Total,2);
                ...
                switch ($afectacion) {
                    ...
                    case 16: # EXPORTACION SE GUARDA COMO INAFECTO
                       ...
                       $total_global   += $valor->CPDEC_Total;
                        break;
                    default:
                        $afecta     = " <font color='green'>[GRATUITA]</font>";
                ...
                $records[] = array(
                    ...
                    ++$posDT => $unidad,
                    ++$posDT => $cantidad.$afecta,
                    ++$posDT => $moneda." ".$preciou,
                    ++$posDT => $moneda." ".$total
...
#FIN REPORTE DE VENTAS POR CLIENTE
    ...
    public function registro_ventas_pdf2($tipo_oper, $tipo_doc = "F", $fecha1, $fecha2, $forma_pago="", $consolidado=0, $sucursal='') 
    {
        ...
        $dompdf->render();
        $dompdf->stream("Reporte_ventas_consolidado ".$fecha1.".pdf", array("Attachment" => false));
    }


#exportar productos precios
    public function exportarListaPrecios($flagbs = 'B', $codigo, $nombre, $familia, $marca, $modelo)
    {
        ...
        $titulo_reporte = "LISTADO DE PRECIOS" ;
        $this->excel->setActiveSheetIndex($hoja);
        $lugar=1;
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("A$lugar",  "EMPRESA");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("B$lugar",  $_SESSION['nombre_empresa']);
        ...
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("A$lugar",  "Codigo");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("B$lugar",  "Descripción");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("C$lugar",  "Categoría");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("D$lugar",  "Marca");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("E$lugar",  "Unidad");
        
        $categorias = $this->producto_model->getCategorias();
        $letra      = 6;

        foreach ($categorias as $key => $value) {
            $cat_desc = $value->TIPCLIC_Descripcion;
            $pos = $this->lib_props->colExcel($letra)."$lugar";
            $this->excel->setActiveSheetIndex($hoja)->setCellValue($pos,  $cat_desc);
            $letra++;
            $pos = $this->lib_props->colExcel($letra)."$lugar";
            $this->excel->setActiveSheetIndex($hoja)->setCellValue($pos,  $cat_desc);
            $letra++;
        ...
        $filter->searchCodigoUsuario    = ($codigo=="-")    ? "" : $codigo;
        $filter->searchProducto         = ($nombre=="-")    ? "" : $nombre;
        $filter->searchFamilia          = ($familia=="-")   ? "" : $familia;
        $filter->searchModelo           = ($modelo=="-")    ? "" : $modelo;
        $filter->searchMarca            = ($marca=="-")     ? "" : $marca;
        $filter->searchFlagBS           = ($flagbs=="-")    ? "" : $flagbs;
        ...
        if ( count($listado) > 0 ){
            foreach ($listado as $indice => $valor) {
                ...
                $categoria      = $valor->FAMI_Descripcion;
                ...
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("A$lugar",  $codigo);
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("B$lugar",  $descripcion);
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("C$lugar",  $categoria);
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("D$lugar",  $marca);
                $this->excel->setActiveSheetIndex($hoja)->setCellValue("E$lugar",  $unidad);

                $categorias = $this->producto_model->getPrecios_e($valor->PROD_Codigo);
               ...
               //var_dump($categorias);exit();
                ...
                foreach ($categorias as $i => $v){
                    ...
                    $pos = $this->lib_props->colExcel($letra)."$lugar";
                    $precio= $v->moneda." ".$v->precio;
                    $this->excel->setActiveSheetIndex($hoja)->setCellValue($pos,  $precio);
                    $letra++;
        ...
        $titulo_archivo = "Lista de precios ".date("YmdHis").".xls";
    ...
    public function registro_ventas_txt(){
        ...
        foreach ($info as $key => $valor){
                    $ruc_cliente=$valor->numero_documento_cliente;

            $am=explode("-",$valor->CPC_Fecha);
            $af=explode("-",$valor->CPC_FechaVencimiento);
            //$fecha_emision=$am[2]."/".$am[1]."/".substr($am[0], 2,4);
            //$fecha_v=$af[2]."/".$af[1]."/".substr($af[0], 2,4);
            $fecha_emision=$am[2]."/".$am[1]."/".$am[0];
            $fecha_v=$af[2]."/".$af[1]."/".$af[0];


            $tdoc2=$valor->CPC_TipoDocumento;
            if ($tdoc2=="F") {
                $tdoc="FT";
            }
            if ($tdoc2=="B") {
            ...
            $serie_numero=$valor->CPC_Serie.sprintf('%04d', $valor->CPC_Numero);
            $razon=substr($valor->razon_social_cliente, 0, 59);
            $razon2=substr($valor->razon_social_cliente, 0, 49);


            $direccion="";
            //Anexo Cliente
            $indice = array_search($ruc_cliente,$documentos,false);
            $razon3="";
            $doc_cli="";
            $tipo_persona="01";
            if ($indice==""){

                $datos_cliente=explode("-",$valor->datos_cliente);
                $apellido_p=$datos_cliente[1];
                $apellido_m=$datos_cliente[2];
                $nombre_f=$datos_cliente[0];

                if (strlen($ruc_cliente)==11){

                        if (substr($ruc_cliente, 0, 2)=="10"){

                                $datos_cliente_temp=explode(" ",$valor->razon_social_cliente);
                                $apellido_p=$datos_cliente_temp[0];
                                $apellido_m=$datos_cliente_temp[1];
                                $nombre_f=$datos_cliente_temp[2];
                        }

                        $razon3=substr($valor->razon_social_cliente, 0, 99);
                        $doc_cli=$ruc_cliente;
                        $tipo_persona="02";
                }
            else
            {
              $razon3=substr($valor->razon_social_cliente2, 0, 99);
            }


                if ($datos_cliente[3]==193){
                    $nacionalidad=9589;
                }
                else
                    $nacionalidad="";

                $sexo=$datos_cliente[4]==1?0:1;



                $contenido_cliente.="02|".$ruc_cliente."|".$doc_cli."|".$razon3."|".$direccion."|".$valor->tipo_documento_cliente."|".$ruc_cliente."|".$tipo_persona."|".$apellido_p."|".$apellido_m."|".$nombre_f."||".$nacionalidad."|".$sexo;

                array_push($documentos, $ruc_cliente);
            ...
            $estado_cpc=$valor->CPC_FlagEstado==0?1:0;
            if ($tdoc=="FT") {
                $contenido.="12120001|".$am[0].$am[1]."|03|".sprintf('%04d', $numero)."|".$fecha_emision."|02|".$ruc_cliente."|".$tdoc."|".$serie_numero."|".$fecha_emision."|||".$valor->CPC_igv."|18|".$valor->CPC_total."|VTA|".$valor->CPC_TDC."|||".$estado_cpc."|D|||0|||\r\n"; //Ckeck
                $contenido.="40111000|".$am[0].$am[1]."|03|".sprintf('%04d', $numero)."|".$fecha_emision."|02|".$ruc_cliente."|".$tdoc."|".$serie_numero."|".$fecha_emision."||||18|".$valor->CPC_igv."|VTA|".$valor->CPC_TDC."|||".$estado_cpc."|H|||0|||\r\n";
                $contenido.="70111100|".$am[0].$am[1]."|03|".sprintf('%04d', $numero)."|".$fecha_emision."|02|".$ruc_cliente."|".$tdoc."|".$serie_numero."|".$fecha_emision."||||18|".$valor->CPC_subtotal."|VTA|".$valor->CPC_TDC."|||".$estado_cpc."|H|||0|||";
            }
            if ($tdoc=="BV") {
                $contenido.="12120001|".$am[0].$am[1]."|03|".sprintf('%04d', $numero)."|".$fecha_emision."|02|".$ruc_cliente."|".$tdoc."|".$serie_numero."|".$fecha_emision."|||".$valor->CPC_igv."|18|".$valor->CPC_total."|VTA|".$valor->CPC_TDC."|||".$estado_cpc."|D|||0|||\r\n"; //Ckeck
                $contenido.="40111000|".$am[0].$am[1]."|03|".sprintf('%04d', $numero)."|".$fecha_emision."|02|".$ruc_cliente."|".$tdoc."|".$serie_numero."|".$fecha_emision."||||18|".$valor->CPC_igv."|VTA|".$valor->CPC_TDC."|||".$estado_cpc."|H|||0|||\r\n";
            ...
            if ($tdoc=="NC") {
                $dcr=explode('-',str_replace(" ","",$valor->DOCREF));
                $tdoc_n="BV";
                if (substr($dcr[0], 0,1)=="F"){
                    $tdoc_n="FT";
                }
                $contenido.="12120001|".$am[0].$am[1]."|03|".sprintf('%04d', $numero)."|".$fecha_emision."|02|".$ruc_cliente."|".$tdoc."|".$serie_numero."|".$fecha_emision."|".$tdoc_n."|".$dcr[0].$dcr[1]."|".$valor->CPC_igv."|18|".$valor->CPC_total."|VTA|".$valor->CPC_TDC."|||".$estado_cpc."|D|||0|||\r\n"; //Ckeck
                $contenido.="40111000|".$am[0].$am[1]."|03|".sprintf('%04d', $numero)."|".$fecha_emision."|02|".$ruc_cliente."|".$tdoc."|".$serie_numero."|".$fecha_emision."|".$tdoc_n."|".$dcr[0].$dcr[1]."||18|".$valor->CPC_igv."|VTA|".$valor->CPC_TDC."|||".$estado_cpc."|H|||0|||\r\n";
                $contenido.="70111100|".$am[0].$am[1]."|03|".sprintf('%04d', $numero)."|".$fecha_emision."|02|".$ruc_cliente."|".$tdoc."|".$serie_numero."|".$fecha_emision."|".$tdoc_n."|".$dcr[0].$dcr[1]."||18|".$valor->CPC_subtotal."|VTA|".$valor->CPC_TDC."|||".$estado_cpc."|H|||0|||";
            }
            if ($numero<$total) {
                $contenido.="\r\n";
            }
            $numero++;
                ...
                $nombre_archivo="V_Reporte_".$operacion."_".date("Y_m_d_H_i_s").".txt";
                $archivo = fopen($nombre_archivo, "w");
                fwrite($archivo,"$contenido");
                fclose($archivo);

                $nombre_anexo="ANEX_Reporte_".$operacion."_".date("Y_m_d_H_i_s").".txt";
                $archivo_anexo = fopen($nombre_anexo, "w");
                fwrite($archivo_anexo,"$contenido_cliente");
                fclose($archivo_anexo);
            ...
            {
                header('Content-Disposition: attachment; filename='.basename($nombre_anexo));
                header('Content-Length: ' . filesize($nombre_anexo));
                readfile($nombre_anexo);
            }
