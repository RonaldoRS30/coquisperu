...
class Notacredito extends Controller{
                ...
                if ($estado_nota == 1) {
                        ...
                        if ($tipo_oper == 'V' && $docInicio != "N") {
                            ...
                            if ($respSunat != NULL){
                                ...
                                elseif ( ($respSunat->respuestas_aceptadaporsunat == '0') || ($respSunat->respuestas_aceptadaporsunat == null && $respSunat->respuestas_deta == "Este documento ya existe en [PSE]" )){
                    ...
                    if ($tipo_oper == 'V'){
                        ...
                        if ($respSunat != NULL){
                            ...
                            if ($respSunat->respuestas_aceptadaporsunat == null && $respSunat->respuestas_deta != "Este documento ya existe en [PSE]") {
                                ...
                                $disparador .= "<span class='detallesWrong'><a onclick='consultar_estado($codigo, $item)'>Consultar</a></span><br> <span class='detallesWrong'>Denegado <span class='detallesWrong2'> $respSunat->respuestas_deta </span> </span>";
                            }
                            else{
                                $disparador = "<span class='detallesWrong'><a onclick='consultar_estado($codigo, $item)'>Consultar</a></span><br> <span class='detallesWrong'>Denegado <span class='detallesWrong2'> $respSunat->respuestas_deta </span> </span>";
                                $editar="";
    ...
    public function comprobante_modificar(){
        ...
        $filter->CRED_TipoDocumento_inicio  = $this->input->post('origenDocumento');
        $filter->CRED_NumeroInicio          = $serInicio . " - " . $numInicio; // SERIE Y NUMERO DE FACTURA ****************************** CRED_NumeroInicio        
    ...
    public function datatable_ventana_comprobantes(){
        ...
        $lista_comprobante = $this->notacredito_model->getComprobantes($filter);
    ...
    public function relacionar_comprobante(){
        ...
        if ($datos_comprobante != NULL){
            ...
            $serieInfo  = $this->configuracion_model->obtener_numero_documento($compania, $tipo, $value->CPC_TipoDocumento);
            $serie      = strtoupper($docuOrigen.$serieInfo[0]->CONFIC_Serie);
            $numero     = $serieInfo[0]->CONFIC_Numero + 1;
