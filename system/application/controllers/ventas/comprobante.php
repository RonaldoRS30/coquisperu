...
class Comprobante extends Controller{
            ...
            if ($listado_comprobantes != NULL) {
                foreach ($listado_comprobantes as $indice => $valor) {
                    ...
                    if ($estado == 0){
                        ...
                        if ($tipo_docu!="N" && $tipo_oper=="V") {
                            $xml = "<a href='#' onclick='abrir_xml_envioSunat($codigo);' target='_parent'><img src='" . base_url() . "images/xml.png' width='16' height='16' border='0' title='XML sunat'></a>";
                        }
                    ...
                    $lista[] = array(
                                        ...
                                        ++$posDT => $serie."-".$this->lib_props->getOrderNumeroSerie($numero),
                                        ...
                                        ++$posDT => "<div align='center' class='xmlSunat_$item'> <span class='icon-loading'></span> <span class='xmlSunat_data_$item'>$xml</span> </div>",
    ...
    public function comprobante_nueva($tipo_oper = '', $tipo_docu = '', $sucursal = ''){
        ...
        if($tipoDocumento == '')
        {
            ...
            $data['sucursal']       = $this->compania; //deja la compania inicial al momento de crear nuevo doc
            $data['cambio_comp']    = "0";
            $data['total_det']      = "0";
            $data['codigo']         = $codigo;
            $data['cboObra']        = form_dropdown("obra", array('' => ':: Seleccione ::'), "", " class='comboGrande'  id='obra'");
            $transaccion            = ($tipo_oper=="V") ? " DE VENTA":" DE COMPRA";
            $data['contiene_igv']   = (($comp_confi[0]->COMPCONFIC_PrecioContieneIgv == '1') ? '1' : '0');
            ...
            $data['cboVendedor']    = $this->lib_props->listarVendedores($this->session->userdata('persona'));
            //dd($data['cboVendedor']);
    ...
    public function comprobante_insertar(){
        ...
        if ( $this->input->post("forma_pago") != "" && $this->input->post("forma_pago") != "0" )
        {
            ...
            $fp_monto               = 0;//$this->input->post("montoFP_default");
            if ($fp_monto==0 || $fp_monto=="") {
                $fp_monto = $this->input->post('importetotal');
            }
        ...
        $prodpeso           = $this->input->post('prodpeso');
        ...
        if (is_array($prodcodigo)) {
            foreach ($prodcodigo as $indice => $valor) {
                ...
                $filter->PROD_Peso          = $prodpeso[$indice];
    ...
    public function insertar_guiarem($comprobante){
        ...
        $filter->GUIAREMC_Placa             = "0";
        $filter->GUIAREMC_RegistroMTC       = "-";
        $filter->GUIAREMC_Certificado       = "-";
        $filter->GUIAREMC_Licencia          = "0";
        $filter->GUIAREMC_PersReceDNI       = "00000000";
        $filter->GUIAREMC_NombreConductor   = "-";
        $filter->GUIAREMC_RucEmpresaTransp  = "00000000000";
        $filter->GUIAREMC_PesoTotal         = "0";
        $filter->GUIAREMC_NumBultos         = "0";
        $filter->GUIAREMC_ModTransporte     = "2";
        $filter->GUIAREMC_EmpresaTransp     = "-";
        $filter->GUIAREMC_TipoDocTransp     = "6";
        ##########################################
        ...
        if (count($detalle) > 0) {
            foreach ($detalle as $indice => $valor) {
                ...
                $peso = $valor->PROD_Peso;
                    ...
                    $filterGuia->GUIAREMDETC_Peso = $peso;
                    ###########################################
    ...
    public function comprobante_editar($codigo, $tipo_oper = 'V', $tipo_docu = 'F'){
        ...
        $docurefe_codigo    = $datos_comprobante[0]->CPC_DocuRefeCodigo;
        $observacion        = $datos_comprobante[0]->CPC_Observacion;
        $contiene_igv       = $datos_comprobante[0]->CPC_ModoImpresion;
        $estado             = $datos_comprobante[0]->CPC_FlagEstado;
        ...
        $data['nombre_proveedor']       = $nombre_proveedor;
        //$data['contiene_igv']           = (($comp_confi[0]->COMPCONFIC_PrecioContieneIgv == '1') ? '1' : '0');
        $data['contiene_igv']           = $contiene_igv;
        $oculto                         = form_hidden(array('codigo' => $codigo, 'base_url' => base_url(), 'tipo_oper' => $tipo_oper, 'tipo_docu' => $tipo_docu));
        $data['titulo']                 = "EDITAR " . strtoupper($this->obtener_tipo_documento($tipo_docu));
        $data['tipo_docu']              = $tipo_docu;
        $data['formulario']             = "frmComprobante";
    ...
    public function comprobante_modificar(){
        ...
        $filter->CPC_GuiaRemCodigo  = strtoupper($this->input->post('guiaremision_codigo'));
        $filter->CPC_DocuRefeCodigo = strtoupper($this->input->post('docurefe_codigo'));
        $filter->CPC_ModoImpresion  = $this->input->post('contiene_igv');
        //if ($this->input->post('modo_impresion') != '0' && $this->input->post('modo_impresion') != '')
          //  $filter->CPC_ModoImpresion = $this->input->post('modo_impresion');
   ...
   /**gcbq json verificacion de cantidad por producto y cantidad por serie este metodo lo usa json de inventario guiarem y otros***/
    ...
    public function canjear_documento(){
        ...
        $filter->FORPAP_Monto           = $datos[0]->FORPAP_Monto;
        ...
        $filter->CPC_FechaVencimiento   = $datos[0]->CPC_FechaVencimiento;
        ...
        if (is_array($formPagoFP) and count($formPagoFP) > 0)
        {
            foreach ($formPagoFP as $key => $fpago) 
            {
                $stdFormasPago                  = new StdClass();
                $stdFormasPago->CPP_Codigo      = $comprobante;
                $stdFormasPago->FORPAP_Codigo   = $fpago->FORPAP_Codigo;
                $stdFormasPago->MONED_Codigo    = $fpago->MONED_Codigo;
                $stdFormasPago->monto           = $fpago->monto;
                $stdFormasPago->compro_flag_FechaRegistro = $fpago->compro_flag_FechaRegistro;
                $this->comprobante_formapago_model->insertar($stdFormasPago);
        ...
        if ($enviado["exito"] == true){ 
            ...
            exit('{"result":"success","mensaje":"Documento generado", "serie":"' . $serie . '", "numero":"00' . $numero.'","comprobante":"'.$comprobante.'","tipo_oper":"'.$tipoOperacion.'","tipo_docu":"'.$tipo_docu.'"}');
        }
        else{
            ...
            exit('{"result":"error", "mensaje":"Hubo un problema", "serie":"' . $serie . '", "numero":"00' . $numero.'"}');
        }
    }
