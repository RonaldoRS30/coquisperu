...
class Productoprecio_model extends Model {
    ...
    ############################
        ...
        public function cleanRegistro($producto, $categoria){
            $sql = "DELETE FROM cji_productoprecio WHERE PROD_Codigo = $producto AND TIPCLIP_Codigo = '$categoria'";
            $this->db->query($sql);
        }
        ...
        public function buscar($filter) {
            ...
            $query = $this->db->where('PRODPREC_FlagEstado', '1')->order_by('PRODPREP_Codigo','desc')->get('cji_productoprecio');
            if ($query->num_rows() > 0) {
                return $query->result();
            }else{
                return Null;
