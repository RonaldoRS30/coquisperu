...
class Inventario_model extends model
{
    ...
    {
        $sql = "UPDATE cji_inventariodetalle SET INVD_Cantidad := (INVD_Cantidad + $cantidad) WHERE INVD_Codigo = $id";
    ...
    {
        $sql = "UPDATE cji_almacenproducto SET ALMPROD_Stock := (ALMPROD_Stock + $cantidad) WHERE ALMPROD_Codigo = $id";
        return $this->db->query($sql);
    }
    /** End Luis Valdes **/

    ## Dev: Luis Valdes -> Begin
    public function cargaStock($filter)
    {
        if (
            isset($filter->file) && !empty($filter->file) &&
            isset($filter->ext) && !empty($filter->ext) &&
            isset($filter->ajuste) && !empty($filter->ajuste) &&
            ...
            if (!file_exists($filter->file)) {
                ...
                if ($this->db->query($sql)) {
                    ...
                    if ($this->db->query($sql)) {
                        ...
                        /** Paso 3: Inserta en cji_inventarioajustedetalle los items en estado pendiente **/
                        $sql = "INSERT INTO cji_inventarioajustedetalle
                                SELECT NULL as INVADET_Codigo, ac.INVA_Codigo as INVA_Codigo, $filter->persona as PERSP_Codigo, ac.PROD_Codigo as PROD_Codigo,
                                             (SELECT CASE WHEN SUM(ap.ALMPROD_Stock) IS NOT NULL THEN SUM(ap.ALMPROD_Stock) ELSE 0 END FROM cji_almacenproducto ap WHERE ap.ALMAC_Codigo = ac.ALMAP_Codigo AND ap.PROD_Codigo = ac.PROD_Codigo) as INVADET_Stock,
                                             ac.ALMPC_Cantidad as INVADET_StockAjuste,
                                             '$date' as INVADET_FechaRegistro, NULL as INVADET_FechaModificacion, 1 as INVADET_FlagEstado
                                FROM cji_almacenproducto_carga ac
                                WHERE ac.ALMPC_FlagEstado LIKE '2'
                            ";
                        if ($this->db->query($sql)) {
                            $sql = "SELECT COUNT(*) as cantidad FROM cji_almacenproducto_carga ac WHERE ac.ALMPC_FlagEstado LIKE '2'";
                            if ($this->db->query($sql)->row()->cantidad > 0) {
                                /** Paso 4: Cambia el estado de pendiente a aprobado para no conciderar esos items en una proxima carga **/
                                $sql = "UPDATE cji_almacenproducto_carga pc SET pc.ALMPC_FlagEstado = '1'
                                                    WHERE pc.ALMPC_FlagEstado LIKE '2'
                                                        ...
                                                        AND EXISTS(SELECT d.PROD_Codigo FROM cji_inventarioajustedetalle d WHERE d.INVA_Codigo = pc.INVA_Codigo AND d.PROD_Codigo = pc.PROD_Codigo) ";
                                $this->db->query($sql);

                                $sql = "SELECT COUNT(*) as cantidad FROM cji_almacenproducto_carga ac WHERE ac.ALMPC_FlagEstado LIKE '4'";
                                if ($this->db->query($sql)->row()->cantidad == 0) {
                                    return array('result' => 'success', 'details' => 'Ejecuci¨®n completa.');
                                } else {
                                    return array('result' => 'warning', 'details' => 'Algunos productos no se cargaron por presentar codigos no registrados.');
                                }
                            } else {
                                return array('result' => 'danger', 'details' => 'Los productos no se cargaron por presentar codigos no registrados.');
                            }
                        } else {
            ...
            return array('result' => 'error', 'details' => 'Parametros incompletos. Archivo: (' . ((isset($filter->file)) ? $filter->file : '') . ') Ajuste: (' . ((isset($filter->ajuste)) ? $filter->ajuste : '') . ') almacen: (' . ((isset($filter->almacen)) ? $filter->almacen : '') . ') persona: (' . ((isset($filter->persona)) ? $filter->persona : '') . ')');
        }
    }
    ## Dev: Luis Valdes -> End
