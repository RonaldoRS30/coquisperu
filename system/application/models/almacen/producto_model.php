...
class Producto_model extends model{
    ...
    {
        $sql = "select max(PROD_Codigo) as PROD_CodigoUsuario from cji_producto ";
    ...
    public function insertarNvoRegistro($filter){
        ...
        $companias =  $this->compania_model->listar(NULL);
    ...
    */
    ...
    public function getPrecios($producto){
        ...
        $sql = "SELECT tp.TIPCLIP_Codigo, tp.TIPCLIC_Descripcion, GROUP_CONCAT( CONCAT_WS(' ', m.MONED_Simbolo, FORMAT(pp.PRODPREC_Precio,2) ) SEPARATOR ' <br> ') as precio FROM cji_tipocliente tp LEFT JOIN cji_productoprecio pp ON pp.TIPCLIP_Codigo = tp.TIPCLIP_Codigo AND pp.PROD_Codigo = $producto LEFT JOIN cji_moneda m ON m.MONED_Codigo = pp.MONED_Codigo  WHERE tp.COMPP_Codigo = '$compania' AND tp.TIPCLIC_FlagEstado = 1 AND m.MONED_Codigo=1 GROUP BY tp.TIPCLIP_Codigo ";

        //$sql = "SELECT tp.TIPCLIP_Codigo, tp.TIPCLIC_Descripcion, GROUP_CONCAT( CONCAT_WS(' ', m.MONED_Simbolo, FORMAT(pp.PRODPREC_Precio,2) ) SEPARATOR ' <br> ') as precio FROM cji_tipocliente tp LEFT JOIN cji_productoprecio pp ON pp.TIPCLIP_Codigo = tp.TIPCLIP_Codigo AND pp.PROD_Codigo = $producto LEFT JOIN cji_moneda m ON m.MONED_Codigo = pp.MONED_Codigo WHERE tp.TIPCLIC_FlagEstado = 1  GROUP BY tp.TIPCLIP_Codigo";
        $query = $this->db->query($sql);
    ...
    public function getCategorias(){
        ...
        //COMP ACTUAL
        $sql = "SELECT tp.TIPCLIP_Codigo, tp.TIPCLIC_Descripcion FROM cji_tipocliente tp WHERE tp.COMPP_Codigo = '$compania' AND tp.TIPCLIC_FlagEstado = 1 ";
...
//Inserta proveedores
   ...
   public function nameproducto($codigo){
    ...
    public function getPrecios_n($producto){
        ...
        $sql = "SELECT tp.TIPCLIP_Codigo, tp.TIPCLIC_Descripcion, pp.TIPCLIP_Codigo, pp.PRODPREC_Precio as precio, pp.PRODPREC_PorcGanancia as porcen
                        ...
                        WHERE /*tp.COMPP_Codigo = '$compania' AND*/ tp.TIPCLIC_FlagEstado = 1 AND pp.MONED_Codigo= 1
    ...
    public function getPrecios_e($producto){
        $compania = $this->compania;

        $sql = "SELECT tp.TIPCLIP_Codigo, tp.TIPCLIC_Descripcion, pp.TIPCLIP_Codigo, pp.PRODPREC_Precio as precio, pp.PRODPREC_PorcGanancia as porcen, m.MONED_Simbolo as moneda
                        FROM cji_tipocliente tp
                        LEFT JOIN cji_productoprecio pp ON pp.TIPCLIP_Codigo = tp.TIPCLIP_Codigo AND pp.PROD_Codigo = $producto
                        LEFT JOIN cji_moneda m ON m.MONED_Codigo = pp.MONED_Codigo
