...
class Notacredito_model extends Model
{
    ...
    public function getComprobantes($filter = NULL){
        ...
        $limit = "";
        $order = $order = "ORDER BY cp.CPP_Codigo DESC";
        ...
        $viewNotas = " AND NOT EXISTS(SELECT CRED_ComproInicio FROM cji_nota WHERE CRED_TipoOperacion = '$tipo_oper' AND CRED_ComproInicio = cp.CPP_Codigo AND CRED_FlagEstado <> 0 AND(CRED_TipoNota LIKE 'C' AND DOCUP_Codigo NOT IN(1,2,6) OR CRED_TipoNota LIKE 'C' ))";
        ...
        $sql = "SELECT $cols
                    ...
                    LEFT JOIN cji_empresa e ON e.EMPRP_Codigo=c.EMPRP_Codigo AND " . ($tipo_oper != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . "='1'
                        ...
                        WHERE cp.CPC_TipoOperacion='" . $tipo_oper . "'
                         AND cp.COMPP_Codigo =" . $compania . " " . $where . "  $viewNotas
                    $group_by
                    $order
                    $limit
                ";
    ...
    public function getNotas($filter = NULL){
        $compania = $this->somevar['compania'];
        $limit = ( isset($filter->start) && isset($filter->length) ) ? " LIMIT $filter->start, $filter->length " : "";
        $order = " ORDER BY cp.CRED_Codigo DESC ";//( isset($filter->order) && isset($filter->dir) ) ? "ORDER BY $filter->order $filter->dir " : "";
