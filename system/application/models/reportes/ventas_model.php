...
class Ventas_Model extends Model
{
  ...
  public function ventas_por_producto_de_vendedor($finicio, $ffin){
    ...
    foreach ($vendedoresInfo->result() as $key => $value) {
      ...
      $col .= " pp.PROD_UltimoCosto as costo$key,
...
public function excel_reporte_por_producto_vendedor($finicio, $ffin, $vendedor)
{
    ...
    // Incluimos el campo PROD_Volumen
    $productos = "SELECT pp.PROD_CodigoUsuario, pp.PROD_Nombre, m.MARCC_CodigoUsuario, 
                         pp.PROD_UltimoCosto, pp.PROD_Volumen,
                    (
                        SELECT SUM(cd.CPDEC_Cantidad) FROM cji_comprobantedetalle cd
                        INNER JOIN cji_comprobante c ON c.CPP_Codigo = cd.CPP_Codigo
                        INNER JOIN cji_producto p ON p.PROD_Codigo = cd.PROD_Codigo
                        WHERE c.CPC_Fecha BETWEEN '$finicio 00:00:00' AND '$ffin 23:59:59'
                        AND c.CPC_FlagEstado != 0 AND cd.CPDEC_FlagEstado = 1
                        AND p.PROD_Codigo = pp.PROD_Codigo AND c.CPC_TipoOperacion = 'V'
                        AND c.COMPP_Codigo = $compania $where
                    ) as cantidadTotal,
                    (
                        SELECT SUM(cd.CPDEC_Total) FROM cji_comprobantedetalle cd
                        INNER JOIN cji_comprobante c ON c.CPP_Codigo = cd.CPP_Codigo
                    ...
                    (
                        ...
                        AND p.PROD_Codigo = pp.PROD_Codigo AND c.CPC_TipoOperacion = 'V'
                        AND c.COMPP_Codigo = $compania $where
                        GROUP BY cd.UNDMED_Codigo LIMIT 1
                    ) as unidad
                FROM cji_producto pp
                INNER JOIN cji_productocompania pc ON pc.PROD_Codigo = pp.PROD_Codigo AND pc.COMPP_Codigo = $compania
                LEFT JOIN cji_marca m ON m.MARCP_Codigo = pp.MARCP_Codigo
    ...
    if ($productosInfo->num_rows > 0) {
        foreach ($productosInfo->result_array() as $result) {
            // Calcula el volumen total vendido por producto
            $result['volumenTotal'] = (float)$result['PROD_Volumen'] * (float)$result['cantidadTotal'];
            $data[] = $result;
        }
    }
    return $data;
  ...
  public function ventas_por_vendedor_general($vendedor, $inicio, $fin){
      ...
      $sql = "SELECT SUM(c.CPC_Total) as total, SUM(n.CRED_total) as totalNotas, f.FORPAC_Descripcion,
                ...
                INNER JOIN cji_formapago f ON f.FORPAP_Codigo = c.FORPAP_Codigo
                LEFT JOIN  cji_nota n ON n.CRED_ComproInicio = c.CPP_Codigo AND n.CRED_FlagEstado = 1 AND n.CRED_TipoNota LIKE 'C'
                WHERE c.CPC_FlagEstado != 0 AND c.CPC_TipoOperacion = 'V' AND c.CPC_Vendedor = $vendedor AND c.CPC_Fecha BETWEEN '$inicio 00:00:00' AND '$fin 23:59:59'
    ...
    $sql = "SELECT SUM( IF(c.MONED_Codigo=2,c.CPC_TDC*c.CPC_Total,c.CPC_Total)) as VENTAS, p.PERSC_Nombre as NOMBRE, CONCAT_WS(' ',p.PERSC_ApellidoPaterno, p.PERSC_ApellidoMaterno) as PATERNO
       FROM cji_comprobante c
         INNER JOIN cji_persona p ON c.CPC_Vendedor = p.PERSP_Codigo
           WHERE $compania c.CPC_Fecha BETWEEN DATE('$inicio') AND DATE('$fin') AND c.CPC_FlagEstado != 0 AND c.CPC_TipoOperacion = 'V' GROUP BY c.CPC_Vendedor";
  ...
  public function ventas_por_vendedor_mensual($inicio,$fin, $comp = ""){
    ...
    if($anioFin > $anioInicio){
      ...
      AND c.CPC_FlagEstado != 0 AND c.CPC_TipoOperacion = 'V'
  ...
  public function resumen_ventas_mensual($filter=""){
    ...
    if ($totales == false){
      ...
      FROM cji_nota c
      LEFT JOIN cji_moneda m ON m.MONED_Codigo        = c.MONED_Codigo
      LEFT JOIN cji_formapago fp ON fp.FORPAP_Codigo  = c.CRED_FormaPago
 ...
 public function concar_model($filter='')
  ...
  public function ganancia_global($filter)
  {
        ...
        $sql = "SELECT 
                ...
                CASE 
                WHEN SUM(cd.CPDEC_Costo*cd.CPDEC_Cantidad) IS NULL THEN 0
                ELSE SUM(cd.CPDEC_Costo*cd.CPDEC_Cantidad) 
                END AS costo_total,
                ...
                CASE 
                WHEN SUM(cd.CPDEC_Total) -SUM(cd.CPDEC_Costo*cd.CPDEC_Cantidad) IS NULL THEN 0
                ELSE SUM(cd.CPDEC_Total) -SUM(cd.CPDEC_Costo*cd.CPDEC_Cantidad) 
                END AS utilidad
      ...
      public function productos_vendidos_general($filter = NULL, $onlyRecords = true)
      {
          ...
          if (isset($filter->cliente) && $filter->cliente!="") {
            if ($filter->tipo_oper=="V") {
              ...
              $where  .= " AND c.PROVP_Codigo = $filter->cliente";
              $where1 .= " AND cs.PROVP_Codigo = $filter->cliente";
            }
