...
<style>
    ...
    {
    	background-image: url(<?php echo base_url(); ?>/images/synchronize.png);
    	background-size: 36px;
    	opacity: 0.2;
    }

    .lista_botones #eliminar
    {
        background-image: url(<?php echo base_url(); ?>/images/error.png);
        background-size: 36px;
        opacity: 0.2;
    }
                ...
                <div class="col-sm-12 col-md-12 col-lg-12 pall-0">
                    <div class="acciones">
                        <div id="botonBusqueda">
                            ...
                            <ul id="buscarP3" class="lista_botones">
                                <li id="eliminar" data-e="0" onclick="send_eliminar();">Eliminar Prods</li>
                            </ul>
                        </div>
                    ...
                    <table class="fuente8 display" id="table-productos">
                        <thead>
                            <tr class="cabeceraTabla">
                                <th style="width: 05%" data-orderable="true">CHK</th>
                                ...
                                <!--<th style="width: 05%" data-orderable="false">GUARDAR</th>-->
...
<script language="javascript" >
    ...
    $(document).ready(function() {
        ...
        $('#table-productos').DataTable({
            ...
            serverSide: true,
            ajax:{
                    url : "<?=base_url();?>index.php/almacen/producto/datatable_listaprecios/<?=$flagBS;?>",
                    type: "POST",
                    data: { dataString: "" },
                    beforeSend: function()
                    {
   ...
   function registro_producto(){
    ...
    function listar_eliminar(id)
    {
        if ($("#"+id).prop("checked")){
            lista_canje.push(
                {
                    "ID":id
                    //"Serie":$("#canje"+id).attr("data-serie"),
                    //"Moneda":$("#canje"+id).attr("data-moneda"),
                    //"Documento":$("#canje"+id).attr("data-documento"),
                    //"Razon":$("#canje"+id).attr("data-razon"),
                    //"Total":$("#canje"+id).attr("data-total"),
                    //"Cliente":$("#canje"+id).attr("data-cliente")
                });
        }
        else
        {
            let indice = lista_canje.findIndex(lista_canje => lista_canje.ID === id);
            if (indice>-1){
                lista_canje.splice(indice,1);
            }
        }
        verificar_seleccionados();
    }
    ...
    {
        var total=0;
        $.each(lista_canje,function(){
            var cod_temp = parseInt(this.ID);
            if (cod_temp > 0){
                let indice = lista_canje.findIndex(lista_canje => lista_canje.ID === cod_temp);
                if (indice>-1){
                    $("#"+cod_temp).prop("checked", true);
                    total++;
                }
            }
        });
        if (total>0){
            ...
            $("#eliminar").attr("data-e","1");
        }
        else
        {
             $("#eliminar").css("opacity","0.2");
            $("#eliminar").attr("data-e","0");
        }
    }

    function validar_elim(){
        if (lista_canje.length>0) 
        {
    ...
    function send_eliminar()
    {
        var check=$("#eliminar").attr("data-e");
        if (check==1) 
        {
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>index.php/almacen/producto/eliminar_masivo/",
                data: {'array': JSON.stringify(lista_canje)},     
                success: function(data)
                {
                    var titulo="Productos eliminados";
                    Swal.fire({
                        icon: "success",
                        title: titulo,
                        showConfirmButton: true,
